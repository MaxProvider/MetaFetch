...
================================================================================
 MetaFetch — Video Metadata Fetcher
================================================================================
 Scans a local .mp4 library, identifies Pornhub and XHamster videos by their
 ...
 Workflow:
   1. Pre-scan  — reads all configured folders, classifies video IDs,
                  counts Pornhub / XHamster / already-have / unknown.
 ...
 ID classification:
   ph-prefix or long hex (>=10 chars, not MD5) -> Pornhub
   xh-prefix                                   -> XHamster
...
# ----------------------------------------------------------------------------
# classify_id() — classify a video ID as 'pornhub', 'xhamster' or 'unknown'.
# Returns (site, url) tuple. URL is empty string for unknown IDs.
# Rules: ph-prefix -> Pornhub, xh-prefix -> XHamster,
#        10+ hex chars (not 32) -> Pornhub, else -> unknown.
# ----------------------------------------------------------------------------
def classify_id(vid_id):
    """
    Returns ('pornhub', url) or ('xhamster', url) or ('unknown', None)
    Matches the logic from fetch_metadata_pornhub.sh and fetch_metadata_xhamster.sh
    """
    if not vid_id:
        return 'unknown', None

    # XHamster — starts with 'xh'
    if vid_id.lower().startswith('xh'):
        # XHamster URL needs the full slug — we reconstruct from what we have
        return 'xhamster', vid_id

    # Pornhub — ph-prefixed OR long hex (>=10 chars, hex chars only)
    if vid_id.startswith('ph'):
        url = f"https://www.pornhub.com/view_video.php?viewkey={vid_id}"
        return 'pornhub', url
    ...
    # Anything else — try pornhub as a fallback
    url = f"https://www.pornhub.com/view_video.php?viewkey={vid_id}"
    return 'pornhub', url


# ----------------------------------------------------------------------------
# build_xhamster_url() — constructs an XHamster video URL from the filename.
# Strips the ID bracket, lowercases, replaces non-alphanumeric with hyphens.
# e.g. 'Hot Summer Day [xhTJwau].mp4' -> xhamster.com/videos/hot-summer-day-xhTJwau
# ----------------------------------------------------------------------------
def build_xhamster_url(filename, vid_id):
    """Build XHamster URL from filename title slug + ID."""
    # Strip extension and [ID] from end
    base = re.sub(r'\s*\[[^\]]+\]$', '', os.path.splitext(filename)[0])
    slug = base.lower()
    slug = re.sub(r'[^a-z0-9]+', '-', slug)
    slug = slug.strip('-')
    # XHamster URL format: /videos/title-slug-xhID
    return f"https://xhamster.com/videos/{slug}-{vid_id}"
        ...
        try:
            ...
            for folder in folders:
                ...
                for fname in files:
                    ...
                    if site == 'xhamster':
                        url = build_xhamster_url(fname, vid_id)
                        counts['xhamster'] += 1
                        counts['to_fetch'] += 1
                        counts['files'].append((filepath, 'xhamster', vid_id, url))
                    elif site == 'pornhub':
                        counts['pornhub'] += 1
                        counts['to_fetch'] += 1
                        counts['files'].append((filepath, 'pornhub', vid_id, url))
                    else:
                        counts['unknown'] += 1
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        lbl_sub = QLabel("Video Metadata Fetcher  ·  yt-dlp powered  ·  Pornhub & XHamster")
    ...
    # (total, already have, Pornhub, XHamster, unknown, previously failed,
    ...
    def _prescan_done(self, counts):
        ...
        pornhub   = counts.get('pornhub', 0)
        xhamster  = counts.get('xhamster', 0)
        ...
        msg = (
            ...
            f"<tr><td>🎬 Pornhub IDs detected:</td><td><b>{pornhub}</b></td></tr>"
            f"<tr><td>🎬 XHamster IDs detected:</td><td><b>{xhamster}</b></td></tr>"
...
downloaded videos from Pornhub and XHamster using yt-dlp.</p>
<h3>Quick setup</h3>
<ol>
...
<li>Determines whether it's a Pornhub or XHamster video from the ID format</li>
...
<li>How many Pornhub and XHamster IDs were detected</li>
...
<h3>Rate limiting</h3>
<p>Pornhub and XHamster both rate-limit excessive traffic.
...
""",
            "Pornhub IDs": """
<h2>Pornhub Video IDs</h2>
<p>Pornhub videos downloaded with yt-dlp have IDs in one of two formats:</p>
<ul>
...
<p>Both formats use the same Pornhub URL structure and are handled automatically.</p>
<p>Short purely-numeric IDs (less than 10 digits) are from other sites and are skipped.</p>
""",
            "XHamster IDs": """
<h2>XHamster Video IDs</h2>
<p>XHamster videos downloaded with yt-dlp have IDs starting with <b>xh</b>
...
<p>The script builds the XHamster URL by combining the title slug
